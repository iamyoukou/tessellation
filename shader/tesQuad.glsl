...
vec2 interpolate(vec2 v0, vec2 v1, vec2 v2, vec2 v3)
{
    ...
    vec2 res = v0 * (1.0 - u) * (1.0 - v) + v1 * u * (1.0 - v) + v2 * u * v + v3 * (1.0 - u) * v;
...
vec3 interpolate(vec3 v0, vec3 v1, vec3 v2, vec3 v3)
{
    ...
    vec3 res = v0 * (1.0 - u) * (1.0 - v) + v1 * u * (1.0 - v) + v2 * u * v + v3 * (1.0 - u) * v;
...
void main()
{
    worldPos = interpolate(esInWorldPos[0], esInWorldPos[1], esInWorldPos[2], esInWorldPos[3]);
